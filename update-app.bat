...
echo [1/3] در حال نصب و بروزرسانی پیشنیازها (npm install)...
echo.
call npm install

echo.
echo [2/3] Building the React Web Application...
echo [2/3] در حال بیلد کردن کدهای فرانت اند (npm run build)...
echo.
call npm run build

echo.
echo [3/3] Syncing files with Android Studio via Capacitor...
echo [3/3] در حال همگام سازی فایل ها با اندروید استودیو (Capacitor Sync)...
echo.
